...
func validateBadrequest(req *Request, r *http.Request) string {
	...
	if !models.ValidateTenderServiceType(models.TenderServiceType(req.ServiceType)) && req.ServiceType != "" {
		return "incorrect service type"
	}

	if !models.ValidateTenderStatus(models.TenderStatus(req.Status)) && req.Status != "" {
